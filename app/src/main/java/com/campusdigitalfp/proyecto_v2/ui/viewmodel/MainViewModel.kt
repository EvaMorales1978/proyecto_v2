...
class MainViewModel(private val repository: OdooRepository) : ViewModel() {
    ...
    private fun loadAllData() {
        viewModelScope.launch {
            try {
                val data = repository.fetchInitialData()
                uiState = MainUiState.Success(data)
            } catch (e: Exception) {
                uiState = MainUiState.Error(e.message ?: "Error fatal")
            }
        }
    }
}
