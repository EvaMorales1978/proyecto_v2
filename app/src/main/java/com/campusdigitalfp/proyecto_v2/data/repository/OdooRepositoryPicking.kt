...
class OdooRepositoryPicking {
    ...
    suspend fun UpdateMoveLine(
       ...
       /* suspend fun UpdateMoveLine1(
        ...
        val productQty = moveMap["product_qty"].toIntSafe()
        var qtyDone    = moveMap["qty_done"].toIntSafe()
        if (qtyDone >= productQty) {
            throw IllegalStateException(
                "Cantidad completada ($qtyDone) ya alcanza la demandada ($productQty) " +
                        "para el producto $productId en el picking $pickingId"
            )
        }
        ...
        return if (existingLine != null) {
            ...
            // añadir uno en stock_move
            qtyDone = qtyDone+1
            val writeResultMove = client.write(
                db ,
                uid ,
                pass ,
                "stock.move" ,
                listOf(moveId) ,
                mapOf("quantity_done" to qtyDone)
            )

            Log.d("ODOO_PICKING" , "Write result: $writeResultMove")
